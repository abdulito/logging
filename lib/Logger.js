...
var Logger = oo({
  ...
  reconfig: function(config) {
    ...
    var parent = this._parent(config.parent)
    if (!_.isNil(parent)) {
      // if parent, update name and merge configs
      config.name = parent.name + '.' + config.name
      config = _.assignIn({}, parent.config, config)
    }
    ...
    try {
      if (_.isString(args[0])) {
        ...
        if (args.length > 1) {
          ...
          try {
            args[0] = ejson.stringify(args[0])
          } catch (e) {
            args[0] = util.inspect(args[0], {depth: this._inspectDepth})
          }
        }
      } else {
  ...
  child: function(name, config) {
    ...
    return this._logging.getLogger(_config.name, _config)
  },

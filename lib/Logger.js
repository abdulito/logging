...
var Logger = oo({
  ...
  reconfig: function(config) {
    ...
    var defaultsKey = match(
      config.name, this._logging._config.defaults)
    var loggersKey = match(
      config.name, this._logging._config.loggers)
    var mergedConfig = _.assignIn(
      config, 
      _.omit(_.get(this._logging._config.defaults, defaultsKey, {}), 'name'),
      _.omit(_.get(this._logging._config.loggers, loggersKey, {}), 'name'))
  ...
  child: function(name) {
    ...
    this._logging._loggers[config.name]
    return logger
  },
